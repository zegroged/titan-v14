--------------------------------------------------------------------------------
-- PROJECT TITAN V15: Secure Communication Protocol V3
-- Module: Full-Duplex, Multi-Block Encrypted Communication
--------------------------------------------------------------------------------
-- V3 YENİLİKLER (P2):
--   ★ #22: CBC-MAC → AES-CMAC (NIST SP 800-38B) — CMAC subkey K1/K2
--   ★ #23: 64-bit SeqNum — 2^64 mesaj kapasitesi, anti-replay improvement
--   ★ #27: AES triple-pass hazırlığı (TODO: 3x encrypt chain)
--
-- V2 (P0):
--   1. FULL-DUPLEX: TX ve RX bağımsız FSM'ler
--   2. MULTI-BLOCK: 1-16 blok (16-256 byte payload)
--   3. AES ARBITRATION: Round-robin TX/RX
--   4. P0-1: TRNG IV nonce seeding
--   5. P0-5: Fixed-length 16-blok padding
--
-- WIRE FORMAT v3 (BLACK UART üzerinden):
--   [SOF: 0xAA55AA55] [SEQ: 8 byte] [NBLK: 1 byte] [CT: N*16 byte] [MAC: 16]
--   SEQ = 64-bit monotonic counter (★ P2 #23)
...
entity comm_protocol is
    port (
        ...
        -- ★ V15 P0-1: TRNG IV for nonce seeding (Two-Time Pad koruması)
        trng_iv         : in  std_logic_vector(127 downto 0);  -- TRNG random seed
        ...
        -- ★ FAZ 13.2: AES yön sinyali — TX='0', RX='1'
        aes_direction   : out std_logic        -- CTR nonce space bölünmesi
    );
end comm_protocol;
...
architecture Behavioral of comm_protocol is
    ...
    type rx_state_type is (
        ...
        RX_SEQ,             -- ★ P2 #23: SeqNum oku (8 byte)
    ...
    signal tx_sof_cnt      : integer range 0 to 7 := 0;  -- ★ P2 #23: 0..7 for 8-byte SEQ
    ...
    -- ★ P2 #23: 64-bit SeqNum (2^64 mesaj kapasitesi)
    signal tx_seq_num      : unsigned(63 downto 0) := (others => '0');
    signal tx_active       : std_logic := '0';
    
    -- ★ P2 #22: CMAC subkeys (K1, K2 — NIST SP 800-38B)
    signal cmac_k1         : std_logic_vector(127 downto 0) := (others => '0');
    signal cmac_k2         : std_logic_vector(127 downto 0) := (others => '0');
    signal cmac_keys_valid : std_logic := '0';
    
    -- ★ V15 P0-5: Fixed-length padding — gerçek blok sayısı ızle
    signal tx_real_blocks  : integer range 1 to MAX_BLOCKS := 1;
    ...
    -- ★ P2 #23: 64-bit SeqNum
    signal rx_seq_num      : unsigned(63 downto 0) := (others => '0');
    signal rx_seq_last     : unsigned(63 downto 0) := (others => '1');  -- sentinel
    signal rx_active       : std_logic := '0';
    signal rx_seq_byte_cnt : integer range 0 to 7 := 0;  -- 8-byte SEQ counter
...
begin
    ...
    -- ★ FAZ 13.2: AES direction output = current AES owner
    aes_direction <= aes_owner;  -- TX='0', RX='1'
    ...
    begin
        if kill_signal = '1' then
            ...
            tx_total_blocks <= 1;
            tx_send_blk_idx <= 0;
            -- ★ P0-1 + P2 #23: TRNG seed 64-bit
            tx_seq_num     <= unsigned(trng_iv(63 downto 0));
            tx_active      <= '0';
            tx_aes_req     <= '0';
            ...
            if rst_n = '0' then
                ...
                tx_total_blocks <= 1;
                tx_send_blk_idx <= 0;
                -- ★ P0-1 + P2 #23: TRNG seed 64-bit (sync reset)
                tx_seq_num     <= unsigned(trng_iv(63 downto 0));
                tx_active      <= '0';
                tx_aes_req     <= '0';
                blk_tx_start   <= '0';
                    ...
                    if red_rx_valid = '1' then
                        ...
                        if unsigned(red_rx_byte) >= 1 and
                           unsigned(red_rx_byte) <= MAX_BLOCKS then
                            -- ★ V15 P0-5: Gerçek blok sayısını sakla
                            tx_real_blocks  <= to_integer(unsigned(red_rx_byte));
                            -- Wire üzerinde her zaman 16 blok gönderilir
                            tx_total_blocks <= MAX_BLOCKS;
                            tx_state <= TX_COLLECT;
                            tx_active <= '1';
                        end if;
                ...
                when TX_WAIT_AES =>
                    if aes_ct_valid = '1' and aes_owner = '0' then
                        ...
                        -- ★ P2 #23: MAC XOR with 64-bit SeqNum (64+64=128)
                ...
                when TX_NEXT_BLOCK =>
                    ...
                    elsif tx_block_cnt >= tx_real_blocks then
                        -- ★ V15 P0-5: Padding blok — TRNG verisi ile doldur
                        -- Gerçek veri bitti, kalan bloklar rastgele padding
                        tx_data_block <= trng_iv xor 
                            (std_logic_vector(to_unsigned(tx_block_cnt, 8)) &
                             x"000000000000000000000000000000");
                        tx_byte_cnt <= 0;
                        tx_state <= TX_ENCRYPT;
                    else
                        -- Daha gerçek blok var → sonraki 16 byte'ı topla
                        tx_byte_cnt <= 0;
                        tx_state <= TX_COLLECT;
                    end if;
                ...
                -- ★ P2 #23: SEND_SEQ: Sequence number (8 byte — 64-bit)
                when TX_SEND_SEQ =>
                    if blk_tx_busy = '0' then
                        blk_tx_byte <= std_logic_vector(
                            tx_seq_num(63 - tx_sof_cnt*8 downto 56 - tx_sof_cnt*8));
                        blk_tx_start <= '1';
                        if tx_sof_cnt = 7 then  -- 8 byte (0..7)
                ...
                when TX_DONE =>
                    ...
                    -- ★ P2 #23: 64-bit wrap-around guard
                    -- 64-bit ile ~10^18 mesaj kapasitesi (pratikte tükenmez)
                    if tx_seq_num >= x"FFFFFFFFFFFFFFFE" then
    ...
    begin
        if kill_signal = '1' then
            ...
            rx_out_blk_idx <= 0;
            rx_seq_num     <= (others => '0');
            rx_seq_last    <= (others => '1');  -- Sentinel: 64-bit all-ones
            rx_seq_byte_cnt <= 0;
            rx_active      <= '0';
            rx_aes_req     <= '0';
            sof_shift      <= (others => '0');
            ...
            if rst_n = '0' then
                ...
                -- ★ P2 #23: SEQ: Sequence number oku (8 byte — 64-bit)
                when RX_SEQ =>
                    if blk_rx_valid = '1' then
                        rx_seq_num(63 - rx_seq_byte_cnt*8 downto 56 - rx_seq_byte_cnt*8)
                            <= unsigned(blk_rx_byte);
                        if rx_seq_byte_cnt = 7 then  -- 8 byte (0..7)
                            rx_seq_byte_cnt <= 0;
                            rx_state <= RX_NBLK;
                        else
                            rx_seq_byte_cnt <= rx_seq_byte_cnt + 1;
                        end if;
                    end if;
                ...
                when RX_NBLK =>
                    if blk_rx_valid = '1' then
                        ...
                        if rx_seq_num <= rx_seq_last and rx_seq_last /= x"FFFFFFFFFFFFFFFF" then
                            frame_error <= '1';
                            rx_state <= RX_IDLE;
                        else
                            -- Block count (1..16)
