--------------------------------------------------------------------------------
-- PROJECT TITAN V14: Data Gearbox (Width Converter)
-- Module: 8-bit UART ↔ 128-bit AES Bridge + Flush/PKCS#7 Padding
--------------------------------------------------------------------------------
-- AMAÇ: RED/BLACK domain separation ve veri genişliği dönüşümü
--
-- V14.1 DEĞİŞİKLİK (GAP-3 FIX):
--   flush portu eklendi. Kısmi bloklar PKCS#7 ile doldurularak AES'e
--   gönderilebilir. Son byte = padding sayısı (1..16).
--   Ayrıca unpacker'daki double-driver bug düzeltildi.
--
-- MİMARİ:
--   ┌────────────┐                    ┌─────────────┐
--   │   PC       │                    │    AES      │
...
-- FLUSH: flush='1' -> Kısmi blok PKCS#7 ile doldurularak AES'e gönderilir
--------------------------------------------------------------------------------
...
entity data_gearbox is
    generic (
        -- Watchdog timeout: kısmi blok bekleme süresi
        -- Default: 50_000 = 1ms @ 50MHz
        -- Kısmi veri gelip durduğunda (saldırı veya hata) timeout sonrası:
        --   1) Kısmi blok PKCS#7 ile auto-flush edilir
        --   2) timeout_alarm tetiklenir → kill chain'e bağlanabilir
        TIMEOUT_CYCLES : integer := 50_000
    );
    port (
        clk         : in  std_logic;
        rst_n       : in  std_logic;  -- ★ GİYOTİN (pipeline_enable)
        flush       : in  std_logic;  -- ★ GAP-3 FIX: Kısmi blok gönder (PKCS#7)
        ...
        -- ★ FAZ 3: Watchdog timeout alarm
        timeout_alarm : out std_logic  -- '1' = kısmi blok timeout (kill chain'e)
    );
end data_gearbox;
...
architecture Behavioral of data_gearbox is
    ...
    -------------------------------------------------------------------------
    -- 16 byte biriktiği an veya flush geldiğinde AES'e gönder
    -------------------------------------------------------------------------
    ...
    -------------------------------------------------------------------------
    -- PKCS#7 PAD HESAPLAMA (GAP-3 FIX)
    -------------------------------------------------------------------------
    -- pad_count = 16 - byte_cnt (kaç byte padding gerekli)
    -- byte_cnt=0 ise flush yapılmaz (boş blok göndermek anlamsız)
    -------------------------------------------------------------------------
    signal pad_value  : std_logic_vector(7 downto 0);
    signal padded_blk : std_logic_vector(127 downto 0);
    ...
    -------------------------------------------------------------------------
    -- ★ FAZ 3: WATCHDOG TIMER SİNYALLERİ
    -------------------------------------------------------------------------
    -- Kısmi veri gelip durduğunda timeout sonrası auto-flush + alarm
    -------------------------------------------------------------------------
    signal wdt_counter    : integer range 0 to TIMEOUT_CYCLES := 0;
    signal wdt_timeout    : std_logic := '0';  -- tek cycle pulse
    signal timeout_latch  : std_logic := '0';  -- kalıcı alarm
...
begin
    ...
    tx_start <= tx_start_i;
    tx_byte  <= tx_byte_i;
    timeout_alarm <= timeout_latch;

    -------------------------------------------------------------------------
    -- ★ FAZ 3: WATCHDOG TIMER — Kısmi blok bekleme süresi kontrolü
    -------------------------------------------------------------------------
    -- byte_cnt > 0 ve yeni byte gelmiyorsa sayaç artar.
    -- Sayaç doyarsa wdt_timeout='1' (tek cycle pulse) → packer auto-flush.
    -- timeout_latch kalıcı: bir kez tetiklenince rst_n ile temizlenir.
    -------------------------------------------------------------------------
    process(clk)
    begin
        if rising_edge(clk) then
            if rst_n = '0' then
                ...
                wdt_timeout <= '0';  -- default: pulse off
                
                if byte_cnt = 0 then
                    -- Buffer boş — sayaç sıfır
                    wdt_counter <= 0;
                elsif rx_valid = '1' then
                    -- Yeni byte geldi — sayaç sıfırla
                    wdt_counter <= 0;
                elsif wdt_counter = TIMEOUT_CYCLES - 1 then
                    -- ★ TIMEOUT! Kısmi blok çok uzun bekledi
                    wdt_timeout   <= '1';
                    wdt_counter   <= 0;
                    timeout_latch <= '1';  -- kalıcı alarm
                else
    ...
    -------------------------------------------------------------------------
    -- 1. PACKER (RED -> BLACK) - Plaintext Accumulation + PKCS#7 Flush
    -------------------------------------------------------------------------
    -- PC'den gelen 8-bit byte'ları topla, 16 byte dolduğunda AES'e at
    -- flush='1' geldiğinde kısmi bloğu PKCS#7 ile doldur ve AES'e at
    --
    ...
    --   Eğer 10 byte data varsa, pad_count = 6
    --   Padding bytes: 0x06 0x06 0x06 0x06 0x06 0x06
    --   Alıcı: son byte'ı oku → pad sayısını öğren → kaldır
    -------------------------------------------------------------------------
    process(clk)
        variable v_pad_cnt  : integer range 1 to 16;
        variable v_pad_byte : std_logic_vector(7 downto 0);
        variable v_blk      : std_logic_vector(127 downto 0);
    begin
        if rising_edge(clk) then
            if rst_n = '0' then
                -----------------------------------------------------------------
                ...
                -----------------------------------------------------------------
                -- ★ GAP-3 FIX: FLUSH — Kısmi blok PKCS#7 padding ile gönder
                -- ★ FAZ 3: wdt_timeout da auto-flush tetikler
                -----------------------------------------------------------------
                elsif (flush = '1' or wdt_timeout = '1') and byte_cnt > 0 then
                    -- Padding hesapla: pad_count = 16 - byte_cnt
                    v_pad_cnt  := 16 - byte_cnt;
                    v_pad_byte := std_logic_vector(to_unsigned(v_pad_cnt, 8));

                    -- Mevcut data'yı kopyala
                    v_blk := input_buf;

                    -- Kalan byte'ları PKCS#7 pad değeri ile doldur
                    for i in 0 to 15 loop
                        if i >= byte_cnt then
                            v_blk(127 - i*8 downto 120 - i*8) := v_pad_byte;
                        end if;
                    end loop;
    ...
    begin
        if rising_edge(clk) then
            if rst_n = '0' then
                ...
                -- FIX: internal signal'lar kullan (double-driver bug düzeltildi)
                -----------------------------------------------------------------
                output_buf <= (others => '0');
