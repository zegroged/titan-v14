--------------------------------------------------------------------------------
-- PROJECT TITAN V14/V15: Full-Duplex UART Driver
-- Module: Universal Asynchronous Receiver/Transmitter (8N1) + HW Flow Control
--------------------------------------------------------------------------------
...
--
-- STANDART:
--   - Baud Rate: 115200 (parametrik, 921600'e kadar desteklenir)
--   - Data Bits: 8
--   - Parity: None
--   - Stop Bits: 1
--   - Flow Control: CTS/RTS (V15 Callwhite — opsiyonel, varsayılan devre dışı)
--
-- KRİTİK ÖZELLİKLER:
...
entity uart_driver is
    ...
    port (
        ...
        -------------------------------------------------------------------------
        -- V15 CALLWHITE: Donanımsal Akış Kontrolü (HW Flow Control)
        -------------------------------------------------------------------------
        -- Varsayılan: cts_n='0' → akış kontrolsüz mod (V13/V14 uyumlu)
        -- BLACK UART'ta MCU backpressure için kullanılır
        cts_n     : in  std_logic := '0';  -- Clear-To-Send (active low)
                                           -- '0' = gönderebilirsin
                                           -- '1' = dur, tampon dolu
...
begin
    ...
    -------------------------------------------------------------------------
    -- V15 CALLWHITE: RTS Output
    -------------------------------------------------------------------------
    -- uart_driver seviyesinde her zaman '0' (kabul ederim)
    -- Gerçek backpressure artix7_top'ta gearbox fill level'a bağlanacak
    rts_n <= '0';
            ...
            if rst_n = '0' then
                ...
                case tx_state is
                    ...
                    when IDLE =>
                        ...
                        if tx_start = '1' and cts_n = '0' then
                            -- V15: CTS='1' iken TX başlamaz (MCU buffer dolu)
