...
entity uart_telemetry is
    ...
    port (
        ...
        -- GUVENLiK KiLiDi (Factory Mode Control)
        factory_mode    : in  std_logic;  -- '1' = UART aktif, '0' = High-Z + Silent
        
        -- Monitorlenen Sinyaller
        xor_value       : in  std_logic;       -- Faz dedektoru XOR cikisi
        bucket_level    : in  integer range 0 to 100;  -- Kapasitor voltaji (%)
        system_status   : in  std_logic_vector(1 downto 0);  
                                               -- "00"=ARMED, "01"=DANGER, "10"=DEAD
        
        -- V14 Telemetri Girisleri
        glitch_count_in : in  std_logic_vector(7 downto 0) := (others => '0');
        hmac_busy_in    : in  std_logic := '0';
        trng_healthy_in : in  std_logic := '1';
        fi_busy_in      : in  std_logic := '0';
        spi_error_in    : in  std_logic := '0';  -- ★ 7.3: SPI CRC/cmd error (sticky)
        
        -- UART TX Fiziksel Pin (OBUFT kontrolunde)
        ...
        if rst_n = '0' then
            ...
            case msg_state is
                ...
                when MSG_ASSEMBLE =>
                    ...
                    -- "[TITAN] G:XX H:B T:Y F:B\r\n"
                    -- G=glitch count (hex), H=HMAC busy, T=TRNG health, F=FI busy
                    ...
                    -- G: glitch count high nibble
                    message_buffer(msg_ptr) <= x"47"; msg_ptr := msg_ptr + 1;  -- 'G'
                    message_buffer(msg_ptr) <= x"3A"; msg_ptr := msg_ptr + 1;  -- ':'
                    if unsigned(glitch_count_in(7 downto 4)) < 10 then
                        message_buffer(msg_ptr) <= std_logic_vector(unsigned'(x"30") + resize(unsigned(glitch_count_in(7 downto 4)), 8));
                    else
                        message_buffer(msg_ptr) <= std_logic_vector(unsigned'(x"37") + resize(unsigned(glitch_count_in(7 downto 4)), 8));
                    end if;
                    msg_ptr := msg_ptr + 1;
                    -- G: glitch count low nibble
                    if unsigned(glitch_count_in(3 downto 0)) < 10 then
                        message_buffer(msg_ptr) <= std_logic_vector(unsigned'(x"30") + resize(unsigned(glitch_count_in(3 downto 0)), 8));
                    else
                        message_buffer(msg_ptr) <= std_logic_vector(unsigned'(x"37") + resize(unsigned(glitch_count_in(3 downto 0)), 8));
                    end if;
                    msg_ptr := msg_ptr + 1;
                    message_buffer(msg_ptr) <= x"20"; msg_ptr := msg_ptr + 1;  -- ' '
                    -- H: HMAC busy
                    message_buffer(msg_ptr) <= x"48"; msg_ptr := msg_ptr + 1;  -- 'H'
                    message_buffer(msg_ptr) <= x"3A"; msg_ptr := msg_ptr + 1;  -- ':'
                    if hmac_busy_in = '1' then
                        message_buffer(msg_ptr) <= x"31"; -- '1'
                    else
                        message_buffer(msg_ptr) <= x"30"; -- '0'
                    end if;
                    msg_ptr := msg_ptr + 1;
                    message_buffer(msg_ptr) <= x"20"; msg_ptr := msg_ptr + 1;  -- ' '
                    -- T: TRNG health
                    ...
                    message_buffer(msg_ptr) <= x"20"; msg_ptr := msg_ptr + 1;  -- ' '
                    -- F: FI busy
                    message_buffer(msg_ptr) <= x"46"; msg_ptr := msg_ptr + 1;  -- 'F'
                    message_buffer(msg_ptr) <= x"3A"; msg_ptr := msg_ptr + 1;  -- ':'
                    if fi_busy_in = '1' then
                        message_buffer(msg_ptr) <= x"31"; -- '1'
                    else
                        message_buffer(msg_ptr) <= x"30"; -- '0'
                    end if;
                    ...
                    message_buffer(msg_ptr) <= x"20"; msg_ptr := msg_ptr + 1;  -- ' '
                    -- S: SPI error (★ 7.3)
                    message_buffer(msg_ptr) <= x"53"; msg_ptr := msg_ptr + 1;  -- 'S'
                    message_buffer(msg_ptr) <= x"3A"; msg_ptr := msg_ptr + 1;  -- ':'
                    if spi_error_in = '1' then
                        message_buffer(msg_ptr) <= x"31"; -- '1'
                    else
                        message_buffer(msg_ptr) <= x"30"; -- '0'
                    end if;
