                ...
                case cmd_state is
                    ...
                    when ST_RX_HDR =>
                        if cs_active = '0' then
                            ...
                            if bit_count + 1 = HEADER_BITS then
                                -- Header complete: parse from shift register
                                -- After 56 bits shifted in MSB-first, the
                                -- lowest 56 bits of rx_shift hold the data.
                                -- cmd = bits [55:48], len = bits [47:32], seq = bits [31:0]
                                cmd_reg <= rx_shift(54 downto 48) & mosi_sync(1);
                                len_reg <= unsigned(rx_shift(47 downto 32));
                                seq_reg <= unsigned(rx_shift(31 downto 0));

                                -- Route based on payload length
                                if unsigned(rx_shift(47 downto 32)) = 0 then
                                    cmd_state <= ST_RX_CRC;
                                else
                                    cmd_state <= ST_RX_PLD;
                                end if;
                    ...
                    when ST_RX_PLD =>
                        if cs_active = '0' then
                            ...
                            -- Store byte to payload memory when byte boundary hit
                    ...
                    -------------------------------------------------------
                    when ST_VALIDATE =>
                        if seq_reg <= last_seq and last_seq /= x"00000000" then
                            error_int <= '1';
                            cmd_state <= ST_IDLE;
                        else
                            last_seq  <= seq_reg;
